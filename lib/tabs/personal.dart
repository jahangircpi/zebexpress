...
import 'package:democab/bloc/carbloc.dart';
...
class PersonalRidePage extends StatefulWidget {
  static const String id = 'personalride';
  @override
  _PersonalRidePage createState() => _PersonalRidePage();
}

class _PersonalRidePage extends State<PersonalRidePage>
  ...
  void aboutPromo() {
    ...
    userRef.onValue.listen((event) async {
      ...
      //get car details
      if (event.snapshot.value['promoCode'] == null) {
        setState(() {
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: <Widget>[
                      ...
                      Text(
                        'Where do you want your item picked up?',
                        style:
                          ...
                          itemBuilder: (context, index) {
                            strikepay =
                                Provider.of<AppData>(context, listen: false)
                                    .fare
                                    .promo;
                            p = carBloc.carList.elementAt(index).price *
                                RequestMethods.estimateFare(
                                    tripDirectionDetails,
                                    Provider.of<AppData>(context, listen: false)
                                        .fare);
                            p = (p / 10).ceil() * 10;
                            oldpay = p * 100 / strikepay;
                            return FlatButton(
                              onPressed: () {
                                setState(() {
                                  ...
                                  Provider.of<AppData>(context, listen: false)
                                      .updatePrice(p);
                                  deliveryType = index + 1;
                                  // Provider.of<AppData>(context, listen: false)
                                  //     .updateDeliveryType(index);
                                });
                                print('_______$deliveryType');
                              },
                              child: Container(
  ...
  Future<void> getDirection() async {
    ...
    //var thisFare = await RequestMethods.getFare(context);
    ...
    rideSubscription = rideRef.onValue.listen((event) async {
      ...
      if (status == 'ended') {
        if (event.snapshot.value['fares'] != null) {
          ...
          // var paymRef = FirebaseDatabase.instance
          //     .reference()
          //     .child('rideRequest/${rideRef.key}/payment_method');
          // String pr = paymRef.toString();
          ...
          if (response == 'close') {
            ...
            saveMessage('Your delivery completed');
  ...
  void notifyDriver(NearbyDriver driver) {
    print('2__________$deliveryType');
    DatabaseReference driverTriRef =
        FirebaseDatabase.instance.reference().child('drivers/${driver.key}');
    driverTriRef.once().then((DataSnapshot snapshot) {
      if (snapshot.value != null) {
        if (snapshot.value['type'] == deliveryType) {
